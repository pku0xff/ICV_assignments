...
if __name__ == '__main__':
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            # opt.batchSize --> this_batch_size
            print('[%d: %d/%d] train loss: %f accuracy: %f' % (
                epoch, i, num_batch, loss.item(), correct.item() / float(this_batch_size * 1024)))
            writter.add_train_scalar("Loss", loss.item(), count)
            writter.add_train_scalar("Acc", correct.item() / float(this_batch_size * 1024), count)
            ...
            if i % 10 == 0:
                ...
                print('[%d: %d/%d] %s loss: %f accuracy: %f' % (
                    epoch, i, num_batch, blue('test'), loss.item(), correct.item() / float(this_batch_size * 1024)))
                writter.add_test_scalar("Loss", loss.item(), count)
                writter.add_test_scalar("Acc", correct.item() / float(this_batch_size * 1024), count)
    ...
    print("mIOU for class {}: {}".format(2, np.mean(shape_ious)))
